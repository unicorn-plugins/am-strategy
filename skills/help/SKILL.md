...
| doc-exporter | LOW | .docx/.pptx 변환·인포그래픽 |
...
→ reviewer가 독립 검증 → doc-exporter가 장수 질문 → anthropic-skills:docx/pptx 위임 → 경영진 요약·실무진 상세·발표본 생성
```

### 문제 해결

- 설치/설정 문제 → `/am-strategy:setup` 재실행
- `anthropic-skills` 미설치 시 `/report` 실패 → anthropic-skills 플러그인 설치 후 재시도
